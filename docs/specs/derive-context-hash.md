...
**Errors:**
- Context is empty, odd-length, contains non-hex characters, has a `0x` prefix, or exceeds 1024 bytes (2048 hex characters).
- User rejects the approval dialog.
- Wallet does not support the method.

**User approval required.** The wallet MUST show a confirmation dialog before deriving and returning the value. The dialog SHOULD display the requesting origin and the context bytes.
