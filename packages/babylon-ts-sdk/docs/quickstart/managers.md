...
3. Signs and broadcasts to Bitcoin network
...
| Method | Returns |
|--------|---------|
| `preparePegin()` | `{ btcTxHash, fundedTxHex, vaultScriptPubKey, selectedUTXOs, fee, changeAmount }` |
| `registerPeginOnChain()` | `{ ethTxHash, vaultId }` |
| `signAndBroadcast()` | `btcTxid` (string) |
...
After registering a vault, the vault provider needs your signature on payout transactions. PayoutManager signs these PSBTs.
