...
```ts
optional requireQuotedCommissionBps: boolean;
```
...
### calculatePeginTxHash()

```ts
function calculatePeginTxHash(transactionHex): `0x${string}`;
```
...
```ts
function derivePeginVaultId(peginTxHash, depositor): `0x${string}`;
```
