 ...
 * JavaScript. That takes ~1s on a developer machine and over 6s on a loaded CI
 * runner, so the 5s vitest default is not a safe ceiling for it.
 */
const SCALAR_SWEEP_TIMEOUT_MS = 60_000;
...
describe("assertOnChainBtcPubkey", () => {
  ...
  it("accepts every x-coordinate derived from scalars 1..256", () => {
    for (let scalar = 1; scalar <= SCALAR_SWEEP_COUNT; scalar++) {
