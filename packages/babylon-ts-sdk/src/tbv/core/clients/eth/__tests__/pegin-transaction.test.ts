...
function vaultIdFixtures(
  count: number,
): { peginTxHash: Hex; depositor: Address }[] {
  return Array.from({ length: count }, (_, index) => {
    const peginTxHash = sha256(Uint8Array.of(index & 0xff, index >>> 8));
    return {
      peginTxHash: bytesToHex(peginTxHash),
      depositor: getAddress(bytesToHex(sha256(peginTxHash).slice(0, 20))),
    };
  });
}
