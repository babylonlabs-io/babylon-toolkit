...
async function switchToEnglish(tab: Page): Promise<void> {
  ...
  await english.waitFor({ state: "visible", timeout: 10000 }).catch(() => {});
  await english.click({ force: true, timeout: 6000 }).catch(() => {});
...
async function switchToSignet(tab: Page, origin: string): Promise<void> {
  ...
  await signet.waitFor({ state: "visible", timeout: 8000 }).catch(() => {});
  await signet.click({ force: true, timeout: 6000 }).catch(() => {});
  await sleep(2500);
}
...
export async function setupOKXWallet(context: BrowserContext, mnemonic: string, password: string): Promise<string> {
  ...
  await boxes.first().waitFor({ state: "visible", timeout: 10000 }).catch(() => {});
  ...
  await confirmSeed.click({ force: true, timeout: 6000 }).catch(() => {});
  await sleep(3000);
  await closeForeignTabs(context, tab);
  ...
  if (pwFrame) {
    ...
    await pw.first().waitFor({ state: "visible", timeout: 8000 }).catch(() => {});
  ...
  await tab.locator('[data-testid="home-page-home-root-element-id"]').waitFor({ state: "visible", timeout: 20000 }).catch(() => {});
