 ...
 * to determine provider verification status, active/inactive status, and to
 * override on-chain metadata (name, RPC URL, icon) with trusted values.
 *
 * To add or update a provider, edit verified-providers.json.
...
export interface VerifiedProviderEntry {
  ...
  iconUrl?: string;
  /** Whether this provider is currently active/online */
  active: boolean;
}
