/**
 * PSBT (Partially Signed Bitcoin Transaction) builder for peg-in transactions.
 *
 * This module creates a fundedPSBT from an unfunded transaction template
...
/**
 * Builds a funded PSBT from an unfunded peg-in transaction.
 *
 * Process:
 * 1. Parse unfunded transaction from hex
 * 2. Add inputs for each selected UTXO
 * 3. Keep existing pegin output at index 0
 * 4. Add change output if changeAmount > DUST_THRESHOLD
 * 5. Create PSBT from the funded transaction
 *
 * @param params - PSBT building parameters
...
export function buildPeginPsbt(params: PeginPsbtParams): string {
  ...
  console.log('[DEBUG] buildPeginPsbt input:', {
    unfundedTxHex: unfundedTxHex,
    unfundedTxHexLength: unfundedTxHex.length,
    first20Chars: unfundedTxHex.substring(0, 20),
  ...
  console.log('[DEBUG] Has witness markers:', hasWitnessMarkers);
  ...
  console.log('[DEBUG] Extracted vault output:', {
    version,
    locktime,
    vaultValue,
    vaultScriptHex: scriptHex,
    vaultScriptLength: scriptLen,
  });
  ...
  console.log('[DEBUG] Built transaction:', {
    version: tx.version,
    locktime: tx.locktime,
    inputsCount: tx.ins.length,
    outputsCount: tx.outs.length,
    txHex,
