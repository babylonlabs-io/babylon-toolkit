...
describe("FeatureFlagService", () => {
  ...
  describe("IsBabyStakingEnabled", () => {
    it("should return false when NEXT_PUBLIC_FF_BABY_STAKING is not set", () => {
      expect(FeatureFlagService.IsBabyStakingEnabled).toBe(false);
    });

    it('should return false when NEXT_PUBLIC_FF_BABY_STAKING is set to "false"', () => {
      process.env.NEXT_PUBLIC_FF_BABY_STAKING = "false";
      expect(FeatureFlagService.IsBabyStakingEnabled).toBe(false);
    });

    it('should return true when NEXT_PUBLIC_FF_BABY_STAKING is set to "true"', () => {
      process.env.NEXT_PUBLIC_FF_BABY_STAKING = "true";
      expect(FeatureFlagService.IsBabyStakingEnabled).toBe(true);
    });
  });
