...
export function useIncentiveParams({ enabled = true }: { enabled?: boolean } = {}) {
    return useClientQuery<IncentiveParamsResult>({
        ...
        queryFn: async () => {
            ...
            const result = {
                btcStakingPortion: incentive ? Number(incentive.btcStakingPortion ?? 0) : null,
                fpPortion: incentive ? Number(incentive.fpPortion ?? 0) : null,
                validatorsPortion: costaking ? Number(costaking.validatorsPortion ?? 0) : null,
                costakingPortion: costaking ? Number(costaking.costakingPortion ?? 0) : null,
            };
            console.log("[useIncentiveParams] Result:", result);
            return result;
        },
        enabled,
        staleTime: 5 * 60 * 1000,
