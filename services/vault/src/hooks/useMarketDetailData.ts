 ...
 */
export function useMarketDetailData(marketId: string | undefined): UseMarketDetailDataResult {
    const { address } = useETHWallet();
    const { btcBalance, loading: isBTCBalanceLoading, error: btcBalanceError } = useBTCBalance();

    // Fetch all user positions
    const { positions: allPositions, loading: isPositionsLoading, error: positionsError, refetch: refetchPositions } = useUserPositions(
    ...
    const userPosition = useMemo(() => {
        if (!marketId || !allPositions) {
            console.log("useMarketDetailData: No marketId or positions", { marketId, allPositions });
            return null;
        }
    ...
    const loading = isMarketLoading || isMarketsLoading || isPositionsLoading || isBTCBalanceLoading;

    // Combine errors
    const error = marketError || marketsError || positionsError || btcBalanceError;
