...
export interface PendingPeginRequest {
  ...
  provider?: StoredProvider; // Vault provider information for UI display
}
...
/**
 * Get storage key for a specific Ethereum address
 * The address is already included in the key, so no need to store it in the data
 */
...
export function updatePeginStatus(
  ...
  const updatedPegins = existingPegins.map((p) =>
    p.id === normalizedId ? { ...p, status, ...(btcTxHash && { btcTxHash }) } : p,
  );
  savePendingPegins(ethAddress, updatedPegins);
}
