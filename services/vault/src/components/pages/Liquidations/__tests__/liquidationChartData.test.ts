...
function makeGroup(index: number, overrides: Partial<LiquidationGroup> = {}): LiquidationGroup {
  return {
    index,
...
describe("buildLiquidationChartData", () => {
  it("maps each group to a band with cycling tones and 1-based labels", () => {
    ...
    const { bands } = buildLiquidationChartData(result, { btcPrice: 90000, collateralFactor: CF });

    expect(bands).toHaveLength(3);
    expect(bands.map((b) => b.tone)).toEqual(["1", "2", "3"]);
    expect(bands.map((b) => b.label)).toEqual(["Liq Event 1", "Liq Event 2", "Liq Event 3"]);
  });
  ...
  it("stacks band price extents from each trigger down to the next (last → floor 0)", () => {
    ...
    const { bands, priceAxis } = buildLiquidationChartData(result, { btcPrice: 88400, collateralFactor: CF });
    ...
    expect(priceAxis.map((t) => t.value)).toEqual([88400, 77682, 40283, 3597, 0]);
  });
  ...
  it("marks a band liquidated once the simulated price is at/below its trigger", () => {
    ...
    const { bands } = buildLiquidationChartData(result, { btcPrice: 60000, collateralFactor: CF });
  ...
  it("derives post-liquidation HF from remaining collateral × price × CF ÷ debt", () => {
    const result = makeResult([makeGroup(0, { btcRemainingAfter: 0.5, debtRemainingAfter: 15106 })]);
    const { cards } = buildLiquidationChartData(result, { btcPrice: 88400, collateralFactor: CF });
  ...
  it("badges only the first-seized group as sacrificial", () => {
    const result = makeResult([makeGroup(0), makeGroup(1), makeGroup(2)]);
    const { cards } = buildLiquidationChartData(result, { btcPrice: 90000, collateralFactor: CF });

    expect(cards.map((c) => c.badge)).toEqual(["sacrificial", "protected", "protected"]);
  });

  it("renders the full-liquidation group's fairness as a wBTC payment", () => {
    const result = makeResult([
      makeGroup(0, { isFullLiquidation: true, fairnessPaymentUsd: 81, debtRemainingAfter: 0 }),
    ]);
    const { cards } = buildLiquidationChartData(result, { btcPrice: 40500, collateralFactor: CF });
