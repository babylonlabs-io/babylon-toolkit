...
export function PositionNotificationBanner({
  ...
  if (effectiveStatus === "stale-price") {
    if (statusOverride === "stale-price" || staleBannerReady) {
      return (
        <Notification
          variant="warning"
          title={COPY.positionNotifications.stalePriceTitle}
          data-testid={TEST_ID}
          data-severity="yellow"
        >
          {COPY.positionNotifications.stalePriceDetail}
        </Notification>
      );
    }
  ...
  if (bannerState.severity === "green") {
    title = COPY.positionNotifications.optimalTitle;
    detail = COPY.positionNotifications.optimalDetail;
