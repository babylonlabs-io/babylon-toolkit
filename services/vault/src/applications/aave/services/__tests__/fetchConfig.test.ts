...
describe("fetchAaveAppConfig", () => {
  ...
  it("wraps spoke read failures with adapter context", async () => {
    mockGetCoreSpokeAddress.mockRejectedValueOnce(new Error("rpc down"));
    mockRequest.mockResolvedValueOnce(makeResponse());

    await expect(fetchAaveAppConfig()).rejects.toThrow(
      `Failed to resolve Core Spoke address from adapter ${ENV_ADAPTER}`,
    );
  });
});
