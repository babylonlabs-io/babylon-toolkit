...
describe("getPendingPegins integrity validation", () => {
  ...
  it("filters entries whose selectedUTXOs carries a null internalPubkeyHex", () => {
    // null is JSON-representable, so it survives a round trip. It decides
    // whether the broadcast signs each input under its own key, and reaches
    // the resume path outside its stage error wrapper.
    const tampered = {
      ...validPegin,
      selectedUTXOs: [
        {
  ...
  it("filters entries whose selectedUTXOs internalPubkeyHex is not an x-only key", () => {
    const tampered: PendingPeginRequest = {
      ...validPegin,
      selectedUTXOs: [
        {
          ...
          value: "50000",
          scriptPubKey: "deadbeef",
          internalPubkeyHex: `0x${"ab".repeat(32)}`,
        },
      ],
    };
    localStorage.setItem(storageKey, JSON.stringify([tampered]));
  ...
  it("filters entries whose selectedUTXOs is only partly annotated", () => {
    // One key present puts the broadcast into per-input-key mode for the
    // whole set, where the unannotated entry is refused — so a mixed record
    // is a resume that fails only after the user acts on it.
    const tampered: PendingPeginRequest = {
      ...validPegin,
      selectedUTXOs: [
        {
          ...
          value: "50000",
          scriptPubKey: "deadbeef",
          internalPubkeyHex: "ab".repeat(32),
        },
        {
          txid: VALID_TXID_B,
          vout: 0,
  ...
  it("keeps entries whose selectedUTXOs carries a valid internalPubkeyHex", () => {
    const multiAddress: PendingPeginRequest = {
      ...validPegin,
      selectedUTXOs: [
        {
          txid: VALID_TXID_A,
    ...
    localStorage.setItem(storageKey, JSON.stringify([multiAddress]));
