...
describe('Deposit Calculations', () => {
  describe('calculateDepositFees', () => {
    ...
    it('should calculate higher network fees for multiple UTXOs', () => {
      ...
      expect(feesMultiple.btcNetworkFee).toBeGreaterThan(feesSingle.btcNetworkFee);
  ...
  describe('calculateDepositAmountBreakdown', () => {
    it('should calculate correct breakdown with sufficient balance', () => {
      ...
      expect(breakdown.totalRequired).toBe(depositAmount + breakdown.fees.totalFee);
      expect(breakdown.changeAmount).toBe(availableBalance - breakdown.totalRequired);
    });
