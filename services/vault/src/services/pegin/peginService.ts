...
import { 
  getDepositorPeginRequests,
  getPeginRequestsBulk,
  type PeginRequest 
} from '../../clients/eth-contract/btc-vaults-manager/query';
...
export async function getPeginRequestsWithDetails(
  ...
  const txHashes: Hex[] = await getDepositorPeginRequests(
  ...
  const peginRequests = await getPeginRequestsBulk(
    btcVaultsManagerAddress,
    txHashes
  );
  ...
  const peginRequestsWithDetails = peginRequests.map((peginRequest: PeginRequest, index: number) => ({
    peginRequest,
