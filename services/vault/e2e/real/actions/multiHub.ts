 ...
 */
async function assertLoanRowsNameHubs(
  ctx: ActionContext,
  ...
  for (const reserve of reserves) {
    ...
    await row.waitFor({
      state: "visible",
      timeout: LOAN_ROW_APPEAR_TIMEOUT_MS,
    });
    ...
    if (!text.includes(hubLabel))
      throw new Error(
        `multi-hub: the Loans row for ${describeReserve(reserve)} does not name its hub (row text: "${text.replace(/\s+/g, " ").trim()}").`,
      );
...
export const multiHubAction: Action = {
  ...
  async run(ctx: ActionContext): Promise<void> {
    ...
    try {
      ...
      await assertLoanRowsNameHubs(ctx, reserves);
