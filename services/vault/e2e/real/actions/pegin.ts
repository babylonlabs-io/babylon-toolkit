...
async function waitForDepositCta(
  ...
  while (Date.now() < deadline) {
    ...
    if (label === MAX_VAULTS_CTA_LABEL)
      throw new Error(
        "Maximum BTCVaults reached — the deposit form is blocking new peg-ins for this position (its BTCVault cap is full). Redeem/withdraw a vault or use a different ETH account.",
      );
    if (label === VAULT_COUNT_UNAVAILABLE_CTA_LABEL)
      throw new Error(
        "The deposit form could not verify this position's BTCVault count and is blocking the deposit — retry once the cap read recovers.",
      );
    if (
...
export async function runPeginFlow(
  ...
  log(
    `✅ Pegin complete: ${split ? "two BTCVaults" : "BTCVault"} activated and shown on the dashboard.`,
  );
}
