...
export async function runE2E(config: RunConfig): Promise<void> {
  ...
  try {
    ...
    // can hold; the app blocks the deposit ("Maximum BTCVaults reached") once full. Read the cap +
    ...
    if (willPegin) {
      const cap = await fetchVaultCountCap(
        ...
        artifacts.log(
          `⚠️ Could not pre-check the BTCVault count cap (${error instanceof Error ? error.message : error}); the form will gate it.`,
        );
        return undefined;
      });
      if (cap) {
        ...
        if (cap.maxVaults > 0 && cap.currentCount + needed > cap.maxVaults)
          throw new Error(
            `Maximum BTCVaults reached: this position already holds ${cap.currentCount} of ${cap.maxVaults} BTCVaults` +
    ...
    if (borrowsAgainstExistingCollateral) {
      ...
      if (borrowContext) {
        if (!borrowContext.hasCollateral)
          throw new Error(
            "No collateral to borrow against: this position holds no active BTCVaults. Peg in first, or re-run with --pegin-first (or borrow with a different ETH account that has collateral).",
          );
        artifacts.log(
    ...
    if (config.action === "withdraw" && !config.peginFirst) {
      ...
      if (withdrawContext) {
        if (!withdrawContext.hasCollateral)
          throw new Error(
            "No collateral to withdraw: this position holds no active BTCVaults. Peg in (and activate) first, re-run with --pegin-first, or withdraw with a different ETH account that has collateral.",
          );
