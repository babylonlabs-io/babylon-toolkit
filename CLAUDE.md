...
- Files (all marked `@stability frozen` in JSDoc):
  ...
  - `packages/babylon-ts-sdk/src/tbv/core/vault-secrets/deriveWotsSeed.ts` — `deriveWotsSeed`, `WOTS_SEED_APP_NAME` (`"babylon-btc-vault-wots"`), HKDF-Expand-SHA-256 info string `"babylon-btc-vault-wots-seed"`
...
- These functions call `wallet.deriveContextHash` directly with per-purpose labels (auth, hashlock, wots). The WOTS helper additionally performs JS-side HKDF-Expand-SHA-256 to stretch the wallet's 32-byte output into the 64 bytes that `babe::wots` requires — expansion is contained to the WOTS purpose label, so a fooled approval still cannot leak auth or hashlock material. Any byte-level change to a label, to the WOTS HKDF info string, or to the per-vault context layout rotates the secrets and **invalidates every existing deposit** — users cannot derive matching keys, cannot activate, cannot resume.
- **Rule:** Treat as a hard fork. Changes require: (a) updated golden-vector tests in both this repo and `btc-vault`, (b) a migration plan for in-flight deposits. Match the Rust `babe::wots` reference byte-for-byte for WOTS chain logic. Two-vault test (overlapping inputs, distinct keys) is mandatory for any chain-logic change. Per-purpose isolation is the security property: the WOTS HKDF expansion is the **only** sanctioned JS-side stretch — it stays bound to a single purpose label. Never combine multiple secret types under one `deriveContextHash` call or expand one wallet output into more than one secret type (that would re-introduce the blast-radius issue from the security audit).
