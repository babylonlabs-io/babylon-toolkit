...
- **Rule:** TLV encodings and APDU layouts must cite their firmware/reference-client source and carry golden-vector tests. The envelope gate must reject out-of-range terms with the seam's typed error BEFORE any device I/O. Never log payload bytes. For generated golden vectors — the JSON files under `src/vendor/ledger-bitcoin/__tests__/vectors/` — "explain every changed line" is satisfied by provenance: the reviewer regenerates the whole tree into a clean directory using the oracle pinned in `scripts/requirements.txt` and the firmware-fixtures repo/commit recorded in the vendor README ("Regenerating the vectors"), confirms byte-identity with the committed tree (no changed, missing, or extra files beyond the documented exception), and states that result in the review — provenance asserted in a PR description is not provenance. The generators (`scripts/gen_*.py`), `scripts/requirements.txt`, and the recorded pins are source, not fixtures: line-walk them under this section even though they live outside `src/`, and treat any change that alters regenerated bytes — including an oracle-version or fixtures-commit bump — as a change to the vectors themselves. Everything else under `__tests__/`, including golden-test code and hand-recorded expectation values, is line-walked normally.
